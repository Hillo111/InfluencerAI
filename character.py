...
class Character:
    ...
    class Archetype(Enum):
        LOSER, AVERAGE, NERD, E_KID, PERFECT = range(5)
    ...
    @staticmethod
    # Defaults to AVERAGE
    # Unless traits are archetype specific, traits are drawn from the average archetype
    def make_from_archetype(archetype: Archetype):
        c = Character(
            Character.Anatomy(
                True,
                16,
                Character.Anatomy.Height.AVERAGE,
                Character.Anatomy.Body.AVERAGE,
                "brown",
                "black",
                "white",
                "white"
        ...
        match archetype:
            case Character.Archetype.NERD:
                c.Anatomy.sex = True
                c.Anatomy.body = Character.Anatomy.Body.SKINNY
                c.personality.writing_style = 'annoying'
                c.personality.sociability = 'shy'
                c.personality.picture_presence = 'shy'
                c.description.friends = Count.FEW
                hobbies = ['programming', 'writing', 'video games']
                random.shuffle(hobbies)
                c.preferences.hobbies = hobbies[:random.randint(2, 3)]
            case Character.Archetype.LOSER:
                pass
            case Character.Archetype.E_KID:
                pass
            case Character.Archetype.PERFECT:
                pass
        return c
...
if __name__  == '__main__':
    c : Character = Character.make_from_archetype(Character.Archetype.AVERAGE)
    conv = Conversation(c)
    conv.chat_history = [
        Message(-1, 'Hi how are you?'),
        Message(0, "I'm great, how about you?"),
        Message(-1, "I'm fine, what are you doing right now?")
    ]
    print(conv.talk())